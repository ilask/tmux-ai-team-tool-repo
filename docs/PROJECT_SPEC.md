...
*   **Phase 5: Agent Teams (相互通信) の実装**
    *   各アダプターへのシステムプロンプトの注入機構
    *   LLM出力から宛先を判定しルーティングを変更する Inter-Agent Router
    *   会話スレッド管理 (ThreadID, MessageID の導入)
...
## 4. Gemini CLI, Claude Code, Codex CLI 協業のAgent Teamsユースケース
...
*   **ユースケースA: 高速プロトタイピングとアーキテクチャ設計**
    *   **Gemini CLI (リード/設計):** ユーザーと対話し、要求仕様からアーキテクチャ設計とタスク分割を行う。Web検索能力を活かして最新のライブラリ情報も収集。
